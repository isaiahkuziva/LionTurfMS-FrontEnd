...
const Admins = () => {
  ...
  const [numInvoices, setNumInvoices] = useState(loaderData?.numInvoices || 0);
  ...
  const [data, setData] = useState(() => {
    const temp = loaderData?.invoices || [];
    return temp;
  });
  ...
  useEffect(() => {
    ...
    API.get(INVOICE_ENDPOINTS.get)
      .then((response) => {
        setData(response.data?.invoices);
        setNumInvoices(response.data?.numInvoices);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if (keyword?.length && keyword?.length > 3) {
      setLoading(true);
      API.get(`${INVOICE_ENDPOINTS.get}?keyword=${keyword}&invoiceId=${invoiceId}`)
        .then((response) => {
          setData(response.data?.invoices);
        })
        .catch((error) => {
  ...
  const handlePage = (value) => {
    ...
    setLoading(true);
    API.get(`${INVOICE_ENDPOINTS.get}?page=${page}&${invoice}`)
      .then((response) => {
        setData(response.data?.invoices);
      })
      .catch((error) => {
      ...
      buttons: [
        {
          ...
          onClick: () => {
            dispatch(showLoading());
            API.delete(`${INVOICE_ENDPOINTS.delete}${id}`)
              .then(() => {
            ...
            buttons: [
              {
                ...
                onClick: () => {
                  dispatch(showLoading());
                  API.patch(`${INVOICE_ENDPOINTS.update}${_id}`, {
                    status,
                    permission,
                  })
                    .then(() => {
