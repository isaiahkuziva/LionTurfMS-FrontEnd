...
const AddInstrument = ({ showModal, setShowModal, currentTheme, navigate, }) => {
  ...
  const [isInstrumentAdded, setIsInstrumentAdded] = useState('false')
  ...
  const submit = (e) => {
    ...
    if (values && currentUser) {
      ...
      API.post(`${INSTRUMENT_ENDPOINTS.add}`, requestData)
        .then(() => {
          enqueueSnackbar(`Instrument successfully added`, { variant: "success" });
          setValues(null);
          setShowModal(false);
        })
        .catch((error) => {
