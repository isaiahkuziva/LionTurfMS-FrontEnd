...
const Admins = () => {
  ...
  const [numCertificates, setNumCertificates] = useState(loaderData?.numCertificates || 0);
  ...
  const [data, setData] = useState(() => {
    const temp = loaderData?.certificates || [];
    return temp;
  });
  ...
  useEffect(() => {
    ...
    API.get(CERTIFICATE_ENDPOINTS.get)
      .then((response) => {
        setData(response.data?.certificates);
        setNumCertificates(response.data?.numCertificates);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if (keyword?.length && keyword?.length > 3) {
      setLoading(true);
      API.get(`${CERTIFICATE_ENDPOINTS.get}?keyword=${keyword}&certificateId=${certificateId}`)
        .then((response) => {
          setData(response.data?.certificates);
        })
        .catch((error) => {
  ...
  const handlePage = (value) => {
    ...
    setLoading(true);
    API.get(`${CERTIFICATE_ENDPOINTS.get}?page=${page}&${certificateId}`)
      .then((response) => {
        setData(response.data?.certificates);
      })
      .catch((error) => {
      ...
      buttons: [
        {
          ...
          onClick: () => {
            dispatch(showLoading());
            API.delete(`${CERTIFICATE_ENDPOINTS.delete}${id}`)
              .then(() => {
            ...
            buttons: [
              {
                ...
                onClick: () => {
                  dispatch(showLoading());
                  API.patch(`${CERTIFICATE_ENDPOINTS.update}${_id}`, {
                    status,
                    permission,
                  })
                    .then(() => {
    ...
    <div className={``}>
      ...
      >
        <DataGrid
          ...
          processRowUpdate={editCertificate}
        />
      </div>
