...
const Admins = () => {
  ...
  const [numCustomers, setNumCustomers] = useState(loaderData?.numCustomers || 0);
  ...
  const [data, setData] = useState(() => {
    const temp = loaderData?.customers || [];
    return temp;
  });
  ...
  useEffect(() => {
    ...
    API.get(CUSTOMER_ENDPOINTS.get)
      .then((response) => {
        setData(response.data?.customers);
        setNumCustomers(response.data?.numCustomers);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if (keyword?.length && keyword?.length > 3) {
      setLoading(true);
      API.get(`${CUSTOMER_ENDPOINTS.get}?keyword=${keyword}&name=${name}`)
        .then((response) => {
          setData(response.data?.customers);
        })
        .catch((error) => {
  ...
  const handlePage = (value) => {
    ...
    setLoading(true);
    API.get(`${CUSTOMER_ENDPOINTS.get}?page=${page}&${name}`)
      .then((response) => {
        setData(response.data?.customers);
      })
      .catch((error) => {
      ...
      buttons: [
        {
          ...
          onClick: () => {
            dispatch(showLoading());
            API.delete(`${CUSTOMER_ENDPOINTS.delete}${id}`)
              .then(() => {
            ...
            buttons: [
              {
                ...
                onClick: () => {
                  dispatch(showLoading());
                  API.patch(`${CUSTOMER_ENDPOINTS.update}${_id}`, {
                    status,
                    permission,
                  })
                    .then(() => {
